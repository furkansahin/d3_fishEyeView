(function() {
  ...
  d3.behavior.zoom = function() {
    var translate = [ 0, 0 ], translate0, translate0_array, scale = 1, scale0, scaleExtent = d3_behavior_zoomInfinity, event = d3_eventDispatch(zoom, "zoom"), x0, x1, y0, y1, touchtime;
	var self = this;
    ...
    function zoom() {
	  ...
	  this.on("mousedown.zoom", mousedown).on("mousewheel.zoom", mousewheel).on("mousemove.zoom", mousemove).on("DOMMouseScroll.zoom", mousewheel).on("dblclick.zoom", dblclick);
	  }
	  else{
		  this.on("touchstart.zoom", touchstart).on("touchend.zoom", touchend).on("touchmove.zoom", touchmove).on("touchcancel.zoom", touchcancel);
		  }//.on("touchend.zoom", touchend).on("touchmove.zoom", touchmove);
    }
    ...
    function location(p) {
	 // console.log('location:p=[' + p +'] translate=[' + translate + '] scale=' + scale);
      return [ (p[0] - translate[0]) / scale, (p[1] - translate[1]) / scale ];
    }
    function point(l) {
		//console.log('location:l=[' + l +'] translate=[' + translate + '] scale=' + scale);
      return [ l[0] * scale + translate[0], l[1] * scale + translate[1] ];
    }
    function scaleTo(s) {
      scale = Math.max(scaleExtent[0], Math.min(scaleExtent[1], s));
	  //console.log('scaleTo: ' + scaleExtent + ' ' + s);
    }
	...
	//  d3.select(window).on("touchmove.zoom", touchmove).on("touchend.zoom", touchend).on("touchcancel.zoom", touchcancel);
	//    d3.select(document).on("touchmove.zoom", touchmove).on("touchend.zoom", touchend).on("touchcancel.zoom", touchend);
	// d3.select("body").on("touchmove.zoom", function(){console.log("body.touchmove");}).on("touchend.zoom", function(){console.log("body.touchmove");});
	// window.focus();
	 d3_eventCancel();
	// d3.event.preventDefault();
		//d3.event.stopPropagation();	
	//d3.event.stopPropagation();		
	 ...
	 // var touches_test = d3.event.changedTouches;
      //  return touches ? d3.touches(target, touches)[0] : d3.mouse(target);
	 touch_move = 0;
	 translate0_array = {};	 
      var touches = d3.touches(this,d3.event.targetTouches);
      scale0 = scale;
	  console.log('zoom.touchstart:' + touches.length);
	 // console.log('zoom.targettouchmove:' + d3.event.targetTouches.length);
		//console.log('zoom.changedtouchmove:' + d3.event.changedTouches.length);
	if(touches.length == 1){
		 translate0 = location(touches[0]);	
	}else if(touches.length == 2){
		translate0_array = {};
		touches.forEach(function(t) {
        	translate0_array[t.identifier] = location(t);	
         });
	}
	console.log("start");
	}
	...
	function touchend(){
		...
		//d3.select(window).on("touchmove.zoom", null).on("touchend.zoom", null).on("touchcancel.zoom", null);
		//d3.select(document).on("touchmove.zoom", null).on("touchend.zoom", null).on("touchcancel.zoom", null);
		//window.blur();
		translate0 = null;
		translate0_array = null;
	}
	
	function touchcancel(){
		console.log('zoom.touchcancel9');
		//d3_eventCancel();
		/*if(touch_move == 1){
		d3.event.preventDefault();
		d3.event.stopPropagation();		
    ...
    function touchmove() {
		...
		//var touches = d3.touches(target,d3.event.touches);
		var touches = d3.touches(this,d3.event.targetTouches);
		console.log('zoom.touchmove:' + touches.length);
		...
		if(touches.length == 1){
			...
			translateTo(touches[0],translate0);
			//dispatch(event_);	
			dispatch(event.of(this, arguments));			
		}
			 ...
			 console.log('SCALE');
			if (!translate0_array || !translate0_array[1]){
				translate0_array = {};
				touches.forEach(function(t) {
        			translate0_array[t.identifier] = location(t);	
            	});
			...
			var pt0 = point(translate0_array[0]),pt1 = point(translate0_array[1]);
		
			var lt0 = translate0_array[0], lt1 = translate0_array[1];
			var old_distance = Math.sqrt(Math.pow(pt0[0]-pt1[0], 2) + Math.pow(pt0[1]-pt1[1],2));
	
			var new_distance = Math.sqrt(Math.pow( p0[0]-p1[0],2) + Math.pow(p0[1]-p1[1],2));
			var delta = new_distance - old_distance;
			if(delta != 0)delta = delta/Math.abs(delta);
		
			  p0 = [(p0[0] + p1[0]) / 2, (p0[1] + p1[1]) / 2];
              lt0 = [(lt0[0] + lt1[0]) / 2, (lt0[1] + lt1[1]) / 2];
			
			scaleTo(Math.pow(2, delta * .02) * scale);
            translateTo(p0, lt0);
          //  dispatch(event_);
		  dispatch(event.of(this, arguments));
		}
