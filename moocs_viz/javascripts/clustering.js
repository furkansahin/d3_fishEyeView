var clustering = function(){
	function stringDistance(str1, str2){
		...
		if(str1 && str2){ 
			...
			for(var j = 0; j < sv1.length; j++){
				for(var k = 0; k < sv2.length; k++){
					if(sv1[j].toLowerCase() == sv2[k].toLowerCase()) count++;
				}
			}
			dist = 1- count/Math.max(sv1.length, sv2.length);
		}else{	
			dist =  1;
		}
	...
	function euclidianDistance (vec1 , vec2, weight) {
		var N = vec1.length ;
		var d = 0 ;
		for (var i = 0 ; i < N ; i++){
			if(typeof vec1[i] == "string"){
				d += Math.pow(stringDistance(vec1[i],vec2[i]),2) * weight[i];
			} 
			else d += Math.pow (vec1[i] - vec2[i], 2) * weight[i]; 
		}
		d = Math.sqrt (d) ;
		return d ;
	}

	function manhattanDistance (vec1 , vec2, weight) {
		var N = vec1.length ;
		var d = 0 ;
		for (var i = 0 ; i < N ; i++){
			if(typeof vec1[i] == "string") d += stringDistance(vec1[i],vec2[i]) * weight[i];
			else d += Math.abs (vec1[i] - vec2[i]) * weight[i]; 			
		}
		return d ;
	}

	function maxDistance (vec1 , vec2, weight) {
		var N = vec1.length ;
		var d = 0 ;
		for (var i = 0 ; i < N ; i++){
			if(typeof vec1[i] == "string") d = Math.max (d , stringDistance(vec1[i],vec2[i]) * weight[i]);
			else d = Math.max (d , Math.abs (vec1[i] - vec2[i]) * weight[i]); 
		}
		return d ;
	}
	...
	function prettyVector(vector) {
		...
		var precision = Math.pow(10, clustering.PRINT_VECTOR_VALUE_PRECISION) ; 
...
//				if (l.length != len) alert(l.length + "     -     " + len +"     -    " + l);
	...
	function agglomerateWithDistance (labels, distances, linkage) {
		
		
		var N = labels.length;
		var dMin = new Array(N) ;
		var cSize = new Array(N) ;
		var matrixObj = new clustering.Matrix(N,N);
		var distMatrix = matrixObj.mtx ;
		var clusters = new Array(N) ;
		
		var flag = new Array(N); //check whether one node is clustered with other nodes.
		...
		for(var j = 0; j < distances.length; j++){
			var link = distances[j];
			var source = link.source;
			var target = link.target;
			var value = link.value;

			distMatrix[source][target] = 1/value;
			distMatrix[target][source] = distMatrix[source][target];
		}
		...
		for (i = 0 ; i < N ; i++) {
			...
			clusters[i][0] = new Node (labels[i], null, null, 0, null) ;
			cSize[i] = 1 ;
		}
		...
		for (p = 0 ; p < N-1 ; p++) {
			...
			newCluster = new Node (-1, c1Cluster, c2Cluster , distMatrix[c1][c2] , null) ;
			...
			for (j = 0 ; j < N ; j++) {
				if (linkage == clustering.SINGLE_LINKAGE) {
					if (distMatrix[c1][j] > distMatrix[c2][j])
						distMatrix[j][c1] = distMatrix[c1][j] = distMatrix[c2][j] ;
				} else if (linkage == clustering.COMPLETE_LINKAGE) {
					if (distMatrix[c1][j] < distMatrix[c2][j])
						distMatrix[j][c1] = distMatrix[c1][j] = distMatrix[c2][j] ;
				} else if (linkage == clustering.AVERAGE_LINKAGE) {
	...
	function agglomerateWithWeight (labels, vectors, weights, distance, linkage) {
		...
		var attr_length = weights.length;

		var c1, c2, c1Cluster, c2Cluster, i, j, p, root , newCentroid ;

		if (distance == clustering.EUCLIDIAN_DISTANCE)
			distance = euclidianDistance ;
		else if (distance == clustering.MANHATTAN_DISTANCE)
			distance = manhattanDistance ;
		else if (distance == clustering.MAX_DISTANCE)
		...
		for (i = 0 ; i < N ; i++) {
			...
			for (j = 0 ; j < N ; j++) {
				if (i == j)
					...
					distMatrix[i][j] = distance(vectors[i] , vectors[j], weights) ;
				}
		...
		for (i = 0 ; i < N ; i++) {
			...
			clusters[i][0] = new Node (labels[i], null, null, 0, new Array(attr_length)) ;
			cSize[i] = 1 ;
		}
	...
	function agglomerate (labels, vectors, distance, linkage) {
		...
		agglomerateWithWeight(labels, vectors, weights,distance, linkage);
	}
	

	function kmeans (k, vectors, weights) {
		var n = vectors.length ;
		if ( k > n ) 
			return null ;
		...
		while (repeat) {
			...
			for (var i = 0 ; i < n ; i++) {
				...
				var best ;
				for (var j = 0 ; j < k ; j++) {
					dist = euclidianDistance (centroids[j], vector, weights)
					if (dist < mindist) {
						mindist = dist ;
						best = j ;
					}
			...
			nb_iters++ ;
			if (nb_iters > clustering.KMEANS_MAX_ITERATIONS)
				repeat = false ;
	...
	function Node (label,left,right,dist, centroid) 
	{
		...
		if (left == null && right == null) {
			...
			if(dist == Infinity)this.depth = Math.max (left.depth , right.depth ) ;
			else this.depth = 1 + Math.max (left.depth , right.depth ) ;
		}
	}
	...
	return { 
		...
		agglomerateWithWeight: agglomerateWithWeight,
		agglomerateWithDistance:agglomerateWithDistance,
		kmeans: kmeans
	}
}() ;


clustering.Matrix.prototype.toString = function() 
{
	var lines = [] ;
...
clustering.Node.prototype.isLeaf = function() 
{
...
clustering.Node.prototype.buildDendogram = function (sep, balanced,withLabel,withCentroid, withDistance)
{
	lines = clustering.generateDendogram(this, sep, balanced,withLabel,withCentroid, withDistance) ;
	return lines.join ("\n") ;	
}
